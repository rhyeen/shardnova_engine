...
class MyFunctionalTester(FunctionalTester):
    ...
    def test_setting_course_already_at_location(self):
        """ Setting a new course while the current course has already
            put us at the destination (new destination on course path).
        """
        self.run_test(self._test_setting_course_already_at_location)

    def _test_setting_course_already_at_location(self, args=None):
        test_config = {
            'preserve_output': True,
            'silence_output': True
        }
        ...
        assert (len(output_records) == 3)
        assert('On course to Star: starS' in output_records[2])
        assert('Distance remaining: 5.0 pu' in output_records[2])
        self.__get_command('set_course')(1)
        assert (len(output_records) == 4)
        assert ('Set destination is within no burn range.' in output_records[3])
        # assert ('Inertia quantum shift complete.' in output_records[3])
        assert ('Orbiting Planet: planetS now.' in output_records[3])
        self.__tick()
        self.__tick()
        self.__get_command('check_course')()
        assert (len(output_records) == 5)
    ...
    def test_setting_course_with_insufficient_fuel(self):
        """ Setting a course, but no fuel to do so.
        """
        self.run_test(self._test_setting_course_with_insufficient_fuel)

    def _test_setting_course_with_insufficient_fuel(self, args=None):
        test_config = {
            'preserve_output': True,
            'silence_output': True,
            'starting_fuel': 1
        ...
        self.__get_command('set_course')(0)
        assert (len(output_records) == 2)
        assert ('Insufficient fuel to reach Star: starS' in output_records[1])
        assert ('requires 1.8 fuel' in output_records[1])
        assert ('1.0 available.' in output_records[1])
        self.__get_command('check_course')()
        assert (len(output_records) == 3)
        assert (output_records[2] == 'No directive set.')
        self.__get_command('set_course')(3)
        assert (len(output_records) == 4)
        assert ('On course to Beacon: beaconS' in output_records[3])
        assert ('0.6/1.0 fuel' in output_records[3])
        assert ('3.0 pu' in output_records[3])
        assert ('ETA: 2 ticks' in output_records[3])
        self.__tick()
        self.__tick()
        assert (len(output_records) == 5)
        assert ('reached Beacon: beaconS' in output_records[4])
        self.__get_command('set_course')(2)
        assert (len(output_records) == 6)
        assert ('Insufficient fuel to reach startingFactoryS' in output_records[5])
        assert ('requires 0.6 fuel' in output_records[5])
        assert ('0.4 available.' in output_records[5])
        self.__get_command('get_map')()
        assert (len(output_records) == 7)
        assert ('Currently orbiting Beacon: beaconS' in output_records[6])

    def test_setting_new_course_with_insufficient_fuel(self):
        """ Setting a course, but no fuel to do so.
        """
        self.run_test(self._test_setting_new_course_with_insufficient_fuel)

    def _test_setting_new_course_with_insufficient_fuel(self, args=None):
        test_config = {
            'preserve_output': True,
            'silence_output': True,
            'starting_fuel': 1
        ...
        self.__get_command('set_course')(3)
        assert (len(output_records) == 2)
        assert ('On course to Beacon: beaconS' in output_records[1])
        assert ('0.6/1.0 fuel' in output_records[1])
        assert ('3.0 pu' in output_records[1])
        assert ('ETA: 2 ticks' in output_records[1])
        self.__tick()
        self.__get_command('set_course')(1)
        assert (len(output_records) == 3)
        assert ('Insufficient fuel to reach Planet: planetS' in output_records[2])
        assert ('requires 1.2 fuel' in output_records[2])
        assert ('0.6 available.' in output_records[2])
        self.__tick()
        assert (len(output_records) == 4)
        assert ('reached Beacon: beaconS' in output_records[3])
...
def run_course_tests(functional_test):
    ...
    functional_test.test_setting_course_already_at_location()
    functional_test.test_setting_course_with_insufficient_fuel()
    functional_test.test_setting_new_course_with_insufficient_fuel()


def run_tests(functional_test):
    """ All test cases should be ran in here for organization.
    """
    run_course_tests(functional_test)
